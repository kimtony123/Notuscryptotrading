...
-- Function to create a new trade
    ...
    local timeCreated = math.floor(NOW / 1800) * 1800  -- Round to the nearest hour
    local contractExpiry = math.floor(((NOW + 1800) + (contractExpiry * 1800)) / 1800) * 1800  -- Round to the nearest hour
...
-- Function to run onTick every 30 minutes
function runOnTickEvery30Minutes()
    ...
    while true do
        ...
        if currentTime - lastRun >= interval then
            onTick()
            lastRun = currentTime
        end  -- Sleep for 60 seconds before checking again
    end
end
...
Handlers.add(
    ...
    function(Msg)
        print("Transferring Tokens: " .. tostring(Quantity))
        ao.send({
            Target = ao.id,
            Action = "Transfer",
            Quantity = math.floor(10),
            Recipient = Msg.From,
        })
...
createTrade(2, 50, "Call", "Bitcoin", "bitcoin", 40000, 0.2)  -- Create a test trade with 2-hour expiry

-- Start the onTick handler every 30 minutes
runOnTickEvery30Minutes()
